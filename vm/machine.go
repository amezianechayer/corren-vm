...
func (m *Machine) ResolveBalances() (chan BalanceRequest, error) {
	...
	go func() {
		...
		for addr, needed_assets := range m.Program.NeededBalances {
			...
			if !ok {
				ch <- BalanceRequest{Error: errors.New("invalid program (set balances: invalid address of account)")}
				return
			}
			if account, ok := (*account).(core.Account); ok {
				...
				for addr := range needed_assets {
					...
					if !ok {
						ch <- BalanceRequest{Error: errors.New("invalid program (set balances: invalid address of monetary)")}
						return
					}
					if mon, ok := (*mon).(core.Monetary); ok {
						...
						ch <- BalanceRequest{Error: errors.New("invalid program (set balances: not a monetary)")}
						return
					}
				}
			} else {
				ch <- BalanceRequest{Error: errors.New("incorrect program (set balances: not an account)")}
				return
			}
		}
	}()
